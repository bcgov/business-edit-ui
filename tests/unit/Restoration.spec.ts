...
xdescribe('Restoration component', () => {
  let wrapper: any
  ...
  beforeEach(async () => {
    ...
    // GET payment fee for immediate restoration
    ...
    // GET payment fee for future restoration
    ...
    // FUTURE: mock GET restoration filing

    // create a Local Vue and install router on it
    const localVue = createLocalVue()
    localVue.use(VueRouter)
    const router = mockRouter.mock()
    await router.push({ name: 'restoration' })
    wrapper = shallowMount(Restoration, { localVue, store, router, vuetify })
  ...
  it('display the fee prices properly', async () => {
    ...
    ).toContain('Certain changes require a Restoration Notice which will incur a $100.00 fee.')
    expect(
      wrapper.find('#intro-text').text().replace(/\s+/g, ' ')
    ).toContain('Choosing a Restoration date and time in the future will incur an additional $100.00 fee.')
    ...
    expect(
      wrapper.find('#intro-text').text().replace(/\s+/g, ' ')
    ).toContain('Certain changes require a Restoration Notice which will incur a fee.')
    expect(
      wrapper.find('#intro-text').text().replace(/\s+/g, ' ')
    ).toContain('Choosing a Restoration date and time in the future will incur an additional fee.')
  })
  ...
  // FUTURE
  xit('loads a draft restoration into the store', async () => {
