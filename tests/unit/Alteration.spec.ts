...
for (const test of dataSets) {
  describe(`Alteration component for ${test.entityType}`, () => {
    let wrapper: any
    ...
    store.state.stateModel.tombstone.businessId = test.businessId
    ...
    beforeEach(async () => {
      ...
      get.withArgs(`https://pay.api.url/fees/${test.entityType}/ALTER`)
      ...
      get.withArgs(`https://pay.api.url/fees/${test.entityType}/ALTER?futureEffective=true`)
        .returns(Promise.resolve({
          data: {
            'filingFees': 100.0,
            'filingType': 'Alteration',
      ...
      // GET business
      get.withArgs(`businesses/${test.businessId}`)
        .returns(Promise.resolve({
          data: {
            business: {
              legalName: 'Mock Business Ltd.',
              legalType: test.entityType,
      ...
      get.withArgs(`businesses/${test.businessId}/aliases`)
        .returns(Promise.resolve({
          data: {
            aliases: [{
      ...
      get.withArgs(`businesses/${test.businessId}/addresses`)
        .returns(Promise.resolve({
          data: {
            registeredOffice: {
              deliveryAddress: {
      ...
      get.withArgs(`businesses/${test.businessId}/directors`)
        .returns(Promise.resolve({
          data: {
            directors: [
              {
      ...
      get.withArgs(`businesses/${test.businessId}/share-classes`)
        .returns(Promise.resolve({
          data: {
            shareClasses: [
              {
      ...
      // GET auth info
      get.withArgs(`https://auth.api.url/entities/${test.businessId}`)
        .returns(Promise.resolve({
          data: {
            contacts: [
              {
      ...
      // GET resolutions
      get.withArgs(`businesses/${test.businessId}/resolutions`)
        .returns(Promise.resolve({
          data: {
            resolutions: [
              {
    ...
    it('loads the entity snapshot into the store', async () => {
      ...
      expect(state.tombstone.businessId).toBe(test.businessId)

      // Validate Business
      expect(state.businessInformation.legalType).toBe(test.entityType)
      expect(state.businessInformation.legalName).toBe('Mock Business Ltd.')
      expect(state.businessInformation.identifier).toBe(test.businessId)

      // Validate Name Translations
      const nameTranslations = test.entityType === 'ULC' ? undefined : 'Mock Business French Ltd.'
      expect(state.nameTranslations[0].name).toBe(nameTranslations)
      ...
      const futureEffectiveFees = test.entityType === 'BEN' ? 0 : 100
      expect(store.state.stateModel.currentFees[0].futureEffectiveFees).toBe(futureEffectiveFees)
    })
    ...
    it('certify text is not prefilled/editable for staff user', async () => {
      ...
      const certifiedBy = test.entityType === 'BEN' ? 'undefined undefined' : 'Jon Doe'
      expect(store.state.stateModel.certifyState.certifiedBy).toBe(certifiedBy)
    })
