<template>
  <section id="business-start-date">
    <v-row no-gutters>
      ...
      <!-- Info tooltip-->
      <v-col cols="9">
        <span v-if="!onEditMode" class="info-text mr-1">{{ resolutionDate }}</span>
        <v-tooltip
          top
...
export default class StartDate extends Mixins(CommonMixin, DateMixin) {
  ...
  // Global setter
  @Action setCorrectionResolutionDate!: ActionBindingIF
  ...
  protected get resolutionDate (): string {
    if (this.isBenCorrectionFiling) {
      if (this.hasResolutionDateChanged) {
  ...
  protected onDoneClicked (): void {
    console.info('this.getBusinessFoundingDate; ', this.getBusinessFoundingDate)
    if (this.newCorrectedStartDate) {
      // For firms only the date is valid
      if (!this.getBusinessFoundingDate ||
        this.newCorrectedStartDate !== this.getBusinessFoundingDate?.slice(0, 10)) {
        this.setCorrectionResolutionDate(this.newCorrectedStartDate)
        this.isCorrected = true
      } else {
  ...
  protected onUndoClicked (): void {
    this.setCorrectionResolutionDate(null)
    this.newCorrectedStartDate = null
  ...
  protected syncValidity (): void {
    ...
    this.setValidComponent({ key: 'invalidResolutionDate', value: isValid })
  }
}
</script>
